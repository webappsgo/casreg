# casreg Support System Specification
...
Support functionality is exposed under casreg's `/v1/support/` route prefix. Exact endpoint paths are defined in `src/main.go`; this section documents behavior only.

### Existing Routes

| Method | Path | Description |
|--------|------|-------------|
| `GET`  | `/v1/support/tickets` | List tickets (own for users; all for agents) |
| `POST` | `/v1/support/tickets` | Create ticket |
| `GET`  | `/v1/support/tickets/{id}` | Get ticket with comments |
| `POST` | `/v1/support/tickets/{id}/comments` | Add comment |
| `GET`  | `/v1/support/docs` | List KB articles |
| `GET`  | `/v1/support/docs/{id}` | Get KB article |

### Routes to Add

Additional endpoints needed by this specification (paths to be finalized in implementation):
- Ticket status update (agent)
- Ticket assignment (agent)
- Ticket tagging (agent)
- Chat session create/join/end
- Bot conversation endpoint
- Canned responses CRUD (admin)
- KB article CRUD (admin)
- Support metrics (admin)
